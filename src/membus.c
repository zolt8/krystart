...
{ /*Fire up the memory bus.*/
	...
	if ((MemDescriptor = shmget((key_t)MemBusKey, MEMBUS_SIZE + sizeof(long) * 2, (ServerSide ? (IPC_CREAT | 0660) : 0660))) < 0)
	{
		if (ServerSide) SpitError("InitMemBus(): Failed to allocate memory bus."); /*should probably use perror*/
						...
						"or the 'krystart' command and are seeing this,\n"
						...
						CONSOLE_COLOR_RED " ** " CONSOLE_ENDCOLOR
						"If you are root/using sudo and are seeing this, either Krystart is not running,\n"
	...
	{ /*Client side stuff.*/
		...
		{ /*Wait for server-side to finish setting up its half, if it was just starting up itself.*/
			...
			{
				SmallError("Cannot connect to Krystart over MemBus, stream corrupted. Aborting MemBus initialization.");
				BusRunning = false;
				memset(&MemBus, 0, sizeof(struct _MemBusInterface));
		...
		{ /*Wait ten seconds for server-side to respond.*/
			if (Inc == 100000)
			{ /*Ten seconds.*/
				SmallError("Cannot connect to Krystart over MemBus, timeout expired. Aborting MemBus initialization.");
...
#define BusDataIs(x) !strncmp(x, BusData, strlen(x))
		...
		if (strchr(TWorker, ':') && strchr(TWorker, '/'))
		{
				...
				&HaltParams.TargetSec, &HaltParams.TargetMonth, &HaltParams.TargetDay, &HaltParams.TargetYear) != 6)
			{
				SpitError("Invalid time signature for HALT/REBOOT/POWEROFF over membus.\n"
							"Please report to Krystart. This is probably a bug.");
	...
	{ /*Restart Krystart from disk, but saves object states and whatnot.
		* Done mainly so we can unmount the filesystem after someone updates /sbin/krystart.*/
		
		/**We set this so when we come back we'll know if we are doing a regular reexec.**/
		setenv("KRYSTARTRXDMEMBUS", "1", true);
