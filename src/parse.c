...
#define ForkFunc() fork()
	...
	* and support shell commands, we need to jump through a bunch of hoops.*/
	...
	{ /*Child does all this.*/
		...
		/*If we are supposed to spawn off as a duty, do this.*/
		...
		{ /*Switch directories if desired.*/
			...
			{ /*Failed to chdir.*/
				fprintf(stderr, "Krystart: Object %s " CONSOLE_COLOR_RED "failed" CONSOLE_ENDCOLOR " to chdir to \"%s\".\n",
	...
	{	
		...
		switch (CurObj->Opts.StopMode)
		{
			case STOP_COMMAND:
			{
				...
				{ /*If we're trying to call Krystart's killall5.*/
					...
					else
					{
						...
						if ((Timer = WhitespaceArg(Arg)))
						{
							...
							{
								char ErrBuf[MAX_LINE_SIZE];
								snprintf(ErrBuf, sizeof ErrBuf, "Non-numeric sleep time for Object %s calling Krystart's killall5.", CurObj->ObjectID);
								SmallError(ErrBuf);
								WriteLogLine(ErrBuf, true);
						...
						if (!AllNumeric(RealArg))
						{
							...
							snprintf(ErrBuf, sizeof ErrBuf, "Bad signal number %s for Object %s calling Krystart's killall5 via KILLALL5."
